...
def process_src_pkl(src_pkl_path, force):
    ...
    if os.path.isfile(dest_pkl_path) and not force:
        raise FileExistsError(f'`{dest_pkl_path}` path already exists. Delete it or re-run this script with `--force` flag')
...
def main(args):
    ...
    for src_pkl_path in src_pkl_paths:
        if src_pkl_path == '': continue
        process_src_pkl(src_pkl_path, args['force'])


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--src_pkl', type=str, required=True, help='comma-separated source data pickle file names (WITHOUT the `data/raw/` prefix)')
    parser.add_argument('--force', action='store_true', help='if true, will force write processed pickle file(s)')
