...
def main():
    ...
    model = STCK_Atten(txt_vocab_size, cpt_vocab_size, args.embedding_dim, txt_word_embeddings,\
                        cpt_word_embeddings, args.hidden_size, label_size, gama=args.gama)
    ...
    writer = SummaryWriter(log_dir=f"results/{run_name}/")
    writer.add_hparams(hparam_dict=vars(args),metric_dict={},run_name=run_name)
    ...
    for epoch in range(args.epoch):
        ...
        writer.add_scalar('Precision/eval', p, epoch)
        writer.add_scalar('Recall/eval', r, epoch)
        writer.add_scalar('MacroF1/eval', f1, epoch)
        
        if f1 > best_score:
            best_score = f1
            torch.save(model.state_dict(), 'results/%s/%d_%s_%s.pt' % (run_name, epoch, 'Model', str(best_score)))
        ...
        writer.add_scalar('Precision/test', test_p, epoch)
        writer.add_scalar('Recall/test', test_r, epoch)
        writer.add_scalar('MacroF1/test', test_f1, epoch)
