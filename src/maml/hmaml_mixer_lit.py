...
def finetune(args, model, train_dataloader, eval_dataloader, test_dataloader, device):
    ...
    path_to_model = os.path.join(
        RUN_BASE_DIR,
        "ft",
        f"{args.dataset_name}{args.target_lang}",
    ...
    result = {
        "examples": len(train_dataloader) * args.batch_size,
        "f1": f1,
        "loss": loss,
    }
    return result
...
def main(args, meta_batch_size=None, adaptation_steps=1, cuda=False, seed=42):
    ...
    summary_fname = os.path.join(
        summary_output_dir,
        f"{f_base_model}_{args.exp_setting}_{args.num_meta_iterations}_{args.shots//2}{few_ft}{f_samples}{aux_la}_{args.target_lang}.json",
    )
    ...
    if args.exp_setting == "hmaml-zeroshot":
        meta_tasks_list = [
            get_dataloader(
                split_name="val",
                config=args,
                train_few_dataset_name=f"{src_dsn}{args.source_lang}",
                lang=args.source_lang,
            ),
            get_dataloader(
                split_name="val",
                config=args,
                train_few_dataset_name=f"{args.dataset_name}{args.aux_lang}",
                lang=args.aux_lang,
            ),
        ]
    ...
    if args.exp_setting == "hmaml-zeroshot":
        if args.finetune_fewshot == "few":
            ft_result = finetune(
                args,
                model,
                train_target_ft_dataloader,
                eval_target_ftdataloader,
